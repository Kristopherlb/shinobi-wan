...
function setupBlueprint(): ReadonlyArray<GraphMutation> {
  const hubVpc = createTestNode({
    id: 'platform:hub-vpc', type: 'platform',
    metadata: { properties: { platform: 'aws-vpc', cidrBlock: '10.0.0.0/16' } },
  });
  const hubPublicSubnet = createTestNode({
    id: 'platform:hub-public-subnet', type: 'platform',
    metadata: { properties: { platform: 'aws-subnet', vpcId: 'platform:hub-vpc', cidrBlock: '10.0.1.0/24', availabilityZone: 'us-east-1a', mapPublicIpOnLaunch: true } },
  });
  const hubPrivateSubnet = createTestNode({
    id: 'platform:hub-private-subnet', type: 'platform',
    metadata: { properties: { platform: 'aws-subnet', vpcId: 'platform:hub-vpc', cidrBlock: '10.0.2.0/24', availabilityZone: 'us-east-1a', mapPublicIpOnLaunch: false } },
  });
  const hubSg = createTestNode({
    id: 'platform:hub-sg', type: 'platform',
    metadata: { properties: { platform: 'aws-security-group', vpcId: 'platform:hub-vpc', description: 'Hub SG' } },
  });
  const spokeAVpc = createTestNode({
    id: 'platform:spoke-a-vpc', type: 'platform',
    metadata: { properties: { platform: 'aws-vpc', cidrBlock: '10.1.0.0/16' } },
  });
  const spokeASubnet = createTestNode({
    id: 'platform:spoke-a-subnet', type: 'platform',
    metadata: { properties: { platform: 'aws-subnet', vpcId: 'platform:spoke-a-vpc', cidrBlock: '10.1.1.0/24', availabilityZone: 'us-east-1a', mapPublicIpOnLaunch: false } },
  });
  const spokeBVpc = createTestNode({
    id: 'platform:spoke-b-vpc', type: 'platform',
    metadata: { properties: { platform: 'aws-vpc', cidrBlock: '10.2.0.0/16' } },
  });
  const spokeBSubnet = createTestNode({
    id: 'platform:spoke-b-subnet', type: 'platform',
    metadata: { properties: { platform: 'aws-subnet', vpcId: 'platform:spoke-b-vpc', cidrBlock: '10.2.1.0/24', availabilityZone: 'us-east-1a', mapPublicIpOnLaunch: false } },
  });
  const transitGw = createTestNode({
    id: 'platform:transit-gw', type: 'platform',
    metadata: { properties: { platform: 'aws-transit-gateway', autoAcceptSharedAttachments: 'disable', dnsSupport: 'enable' } },
  });
  const hubTgwAttach = createTestNode({
    id: 'platform:hub-tgw-attach', type: 'platform',
    metadata: { properties: { platform: 'aws-tgw-vpc-attachment', transitGatewayRef: 'platform:transit-gw', vpcRef: 'platform:hub-vpc', subnetIds: ['platform:hub-private-subnet'] } },
  });
  const spokeATgwAttach = createTestNode({
    id: 'platform:spoke-a-tgw-attach', type: 'platform',
    metadata: { properties: { platform: 'aws-tgw-vpc-attachment', transitGatewayRef: 'platform:transit-gw', vpcRef: 'platform:spoke-a-vpc', subnetIds: ['platform:spoke-a-subnet'] } },
  });
  const spokeBTgwAttach = createTestNode({
    id: 'platform:spoke-b-tgw-attach', type: 'platform',
    metadata: { properties: { platform: 'aws-tgw-vpc-attachment', transitGatewayRef: 'platform:transit-gw', vpcRef: 'platform:spoke-b-vpc', subnetIds: ['platform:spoke-b-subnet'] } },
  });
  const networkFw = createTestNode({
    id: 'platform:network-fw', type: 'platform',
    metadata: { properties: { platform: 'aws-network-firewall', vpcRef: 'platform:hub-vpc', subnetMappings: ['platform:hub-private-subnet'], loggingEnabled: true, deleteProtection: true } },
  });
  const natGw = createTestNode({
    id: 'platform:nat-gw', type: 'platform',
    metadata: { properties: { platform: 'aws-nat-gateway', subnetRef: 'platform:hub-public-subnet', connectivityType: 'public' } },
  });
...
describe('Golden: Blueprint BP-I03 — Networking Hub-Spoke', () => {
  ...
  describe('policy evaluation across packs', () => {
    ...
    it('transit-gateway-auto-accept-enabled does not fire (disabled)', () => {
      ...
      expect(compilation.policy?.violations.filter((v) => v.ruleId === 'transit-gateway-auto-accept-enabled')).toHaveLength(0);
    });
    ...
    it('network-firewall-logging-disabled does not fire (logging enabled)', () => {
      ...
      expect(compilation.policy?.violations.filter((v) => v.ruleId === 'network-firewall-logging-disabled')).toHaveLength(0);
    });
  });
});
