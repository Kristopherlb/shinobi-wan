...
export class EksClusterLowerer implements NodeLowerer {
  ...
  lower(node: Node, context: LoweringContext, _resolvedDeps: ResolvedDeps): ReadonlyArray<LoweredResource> {
    ...
    const endpointPrivateAccess = config['endpointPrivateAccess'] !== undefined
      ? (config['endpointPrivateAccess'] as boolean)
      : true;
    const endpointPublicAccess = config['endpointPublicAccess'] !== undefined
      ? (config['endpointPublicAccess'] as boolean)
      : false;
    const enabledClusterLogTypes = Array.isArray(config['enabledClusterLogTypes'])
