...
export class EksGpuNodeGroupLowerer implements NodeLowerer {
  ...
  lower(node: Node, context: LoweringContext, _resolvedDeps: ResolvedDeps): ReadonlyArray<LoweredResource> {
    ...
    const tags = createStandardTags(node.id, 'aws-eks-gpu-node-group', extraTags);
    ...
    const instanceTypes = Array.isArray(config['instanceTypes'])
      ...
      : ['g5.xlarge'];
    const scalingConfig = (config['scalingConfig'] as Record<string, number>) ?? {
      desiredSize: 1,
      minSize: 0,
      maxSize: 4,
    };
    ...
    const labels = (config['labels'] as Record<string, string>) ?? { 'nvidia.com/gpu': 'true' };
