...
const GATE_REPORT_SCRIPT = path.join(
  ROOT,
  'tools',
  'conformance-gate-report.js',
);
...
function extractTableRows(content, sectionHeading) {
  ...
  for (const line of lines) {
    ...
    if (inTable) {
      ...
      // Tolerate formatter-aligned separator rows (e.g. "| --- | :-- |").
...
function assertRequiredColumns(headerRow) {
  ...
  for (const column of required) {
    if (!headerRow.includes(column)) {
      throw new Error(
        `Roadmap missing required execution control column: ${column}`,
      );
    }
  }
}
...
function assertNonEmptyTableCells(rows, sectionName) {
  for (const row of rows) {
    ...
    if (cols.some((cell) => cell === '-' || cell.length === 0)) {
      throw new Error(
        `Roadmap row in ${sectionName} has empty execution metadata: ${row}`,
      );
    }
  }
}
...
function parseRoadmapGateClaim(content) {
  ...
  if (!claimMatch) {
    throw new Error(
      'Roadmap must declare conformance coverage claim in the format `X/Y` gates.',
    );
  }
  return {
...
function getComputedGateCoverage() {
  const result = spawnSync('node', [GATE_REPORT_SCRIPT], {
    cwd: ROOT,
    encoding: 'utf8',
  });
  if (result.status !== 0) {
    throw new Error(
      `Failed to compute conformance gate coverage: ${result.stderr || result.stdout}`,
    );
  }
  ...
  if (jsonStart === -1) {
    throw new Error(
      'Could not parse conformance gate report output for governance check.',
    );
  }
...
function main() {
  ...
  if (trackA.length < 2 || trackB.length < 2) {
    throw new Error(
      'Roadmap execution control tables are missing or incomplete.',
    );
  }
  ...
  if (
    claimed.covered !== computed.covered ||
    claimed.registered !== computed.registered
  ) {
    throw new Error(
